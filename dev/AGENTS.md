...
- `NAMESPACE` and `man/` are generated by `devtools::document()`; never
...
- Do not allow the developer to proceed with opening a PR if it does not
...
must include a concise entry with the PR number and author username
tagged. Developer-only changes (i.e. updates to CI workflows,
development notes, etc.) should never be included in the changelog.
