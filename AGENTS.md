...
MOObject is an R package that defines an S7 class called `multiOmicDataSet` to
store data and analyses from multi-omic experiments.
Its sister R package is MOSuite, which defines the primary functions for
computational and statistical analyses.
MOObject is intended to be lightweight and minimal, containing only what is
needed to create, read, and write multiOmicDataSet objects.
All other functionality belongs in the MOSuite package.

Development of MOObject follows the R packages 2nd edition (https://r-pkgs.org/),
...
for detailed instructions on how to contribute to MOObject.
